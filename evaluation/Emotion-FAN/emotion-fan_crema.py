...
count = {"HAP": 0,"ANG": 0, "DIS": 0, "FEA": 0, "SAD": 0}
correct_emo = {"HAP": 0,"ANG": 0, "DIS": 0, "FEA": 0, "SAD": 0}
total_emo = {"HAP": 0,"ANG": 0, "DIS": 0, "FEA": 0, "SAD": 0}
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default="/path/to/SEVA/Emotion-FAN/checkpoints_crema/checkpoint_epoch_28_baseline.pth",
    ...
    for idx in tqdm(range(total_videos), desc="Predicting emotions"):
        ...
        if pd.notna(existing) and str(existing).strip() != '':
            continue
        try:
            gt_emotion = df.iloc[idx]['gt_emotion'].strip()
    ...
    final_correct = (valid['gt_emotion'].str.strip() == valid['predicted_emotion'].astype(str).str.strip()).sum()
