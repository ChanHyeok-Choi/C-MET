...
parser.add_argument('--audiofile', type=str, default='', help='Optional: use external audio wav instead of extracting from video');
...
parser.add_argument('--min_track',      type=int, default=1,  help='Minimum facetrack duration');
...
# audio.wav extraction, patched:
if opt.audiofile and os.path.exists(opt.audiofile):
    # Copy the external wav file if it exists.
    import shutil
    shutil.copy(opt.audiofile, os.path.join(opt.avi_dir, opt.reference, 'audio.wav'))
else:
